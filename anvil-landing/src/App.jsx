...
import { ArrowRight, ChevronRight, BarChart, Bot, Link as LinkIcon, MessageSquare, Shield, Sparkles, Rocket } from "lucide-react";
  ...
  <section id={id} className={`py-20 md:py-28 ${className}`}>
    <Container>
      <div className="max-w-3xl">
        {eyebrow && <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-sky-300/80">{eyebrow}</p>}
        {title && <h2 className="text-3xl md:text-5xl font-semibold leading-tight text-white">{title}</h2>}
...
export default function ANVILLanding() {
  ...
  return (
    <div className="min-h-screen scroll-smooth bg-[#0a0f1a] text-white selection:bg-sky-400 selection:text-black">
        ...
        <Container>
          <div className="flex h-16 items-center justify-between">
            ...
            <nav className="hidden items-center gap-6 md:flex">
              <a href="#capabilities" className="text-sm text-white/80 hover:text-white">What you can do</a>
              <a href="#steps" className="text-sm text-white/80 hover:text-white">Four steps</a>
      ...
      {/* FOUR STEPS */}
      <Section id="steps" eyebrow="Four easy steps" title="From intent to action in minutes">
        <ol className="grid gap-6 md:grid-cols-4">
          {[
            { t: "Connect", d: "Create a wallet or connect yours. Read-only first, action when ready." },
            { t: "Profile", d: "Set risk and goals. ANVIL adapts venues and defaults to your vibe." },
            { t: "Plan", d: "See a simple plan with net yield after gas and slippage." },
            ...
            <li key={i} className="relative rounded-2xl bg-white/[0.03] p-6 ring-1 ring-white/10">
              <span className="absolute -top-3 left-4 grid h-8 w-8 place-items-center rounded-full bg-sky-400 text-black text-sm font-bold">{i + 1}</span>
